    ...
    for arg in args:
        if arg.startswith("--") and len(arg) > 3:
            ...
            for sub_arg in arg[1:]:
                if ord(sub_arg) not in range(65, 91) and ord(sub_arg) not in range(97, 123):
                    sub_arg_list.clear()
                    break
                sub_arg_list.append(sub_arg)
        ...
        for index in line_indices:
            ...
            for man_arg in man_args:
                ...
                if not man_arg: 
                    ...
                    break
                if (len(arg) == 1 and arg == man_arg[1:]) or (len(arg) != 1 and man_arg[2:].startswith(arg)):
...
def get_flag_option_indices(man_output):
    ...
    for line_index, man_output in enumerate(man_output):
        ...
        if stripped_line and stripped_line[0] == '-' and (line_index - 1) not in line_indices:
...
def main():
    ...
    man_command_output = subprocess.run(['env', 'COLUMNS=32767', 'man', command], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
